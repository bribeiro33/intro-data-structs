...
private:
  ...
  class PairComp {
      private:
        ...
        Key_compare compareK;
      public:
        bool operator()(Pair_type pair1, Pair_type pair2) const {
          //use your instance like you used less in BST to compare the keys of your pairs
          return compareK(pair1.first, pair2.first);
        }
  };
...
public:
  ...
  Iterator find(const Key_type& k) const{
    std::pair<Key_type, Value_type> dummy_pair = std::make_pair(k, vt);
    if (dummy.find(dummy_pair) == dummy.end()){
      return dummy.end();
    }
    else {
      return dummy.find(dummy_pair);
    }
  }
  ...
  Value_type& operator[](const Key_type& k){
    ...
    //std::pair<Key_type, Value_type> query = std::make_pair(k, vt);
    //Iterator it = find(k);
    if (find(k) != end()){
    ...
    else {  //originally from c++ web, dummy.insert was this->insert
  ...
  std::pair<Iterator, bool> insert(const Pair_type &val){
    ...
    //return iterator to current val and false
    if (dummy.find(val) != dummy.end()){
      return std::make_pair(dummy.find(val), false);
    }

    //if doesn't exist:
    //return iterator to new element and true
    else {
      dummy.insert(val);
      return std::make_pair(dummy.find(val), true);
    }
  }
...
private:
  //Value_type example = Value_type();
  BinarySearchTree<Pair_type, PairComp> dummy;
...
// template <typename Key_type, typename Value_type, typename Key_compare>
// typename Map<Key_type, Value_type, Key_compare>::Iterator Map<Key_type, Value_type, 
//Key_compare>::find(const Key_type& k) const {
//   return this->find(k);
//   return this.find(k);
// }
// //size; 
...
// template <typename Key_type, typename Value_type, typename Key_compare>
// std::pair<typename Map<Key_type, Value_type, Key_compare>::Iterator, bool> 
      //Map<Key_type, Value_type, Key_compare>::insert(const Pair_type &val) {
//   assert(false);
// }
