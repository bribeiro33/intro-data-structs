...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static int size_impl(const Node *node) {
    ...
    return (size_impl(node->left) + 1 + size_impl(node->right));
  }

  //helper func height_impl

  // static int height_help(Node *node){
  //   // if (a >= b){ return a; }
  //   // else { return b; }
  ...
  //     heightLeft = height_impl(node->left)
  //   }
  // }
  ...
  static int height_impl(const Node *node) {
    //height of left subtree or height of right subtree
    if (node == nullptr){ return 0; }

    else { 
      if (height_impl(node->right) < height_impl(node->left)){
        return (height_impl(node->left) + 1);
      }
      else { 
    ...
    // int final_val = 1 + fmax(left_side, right_side);
    // return (final_val);
      
    // take two
    // int val = findMax(height_impl(node->left), height_impl(node->right)) + 1;
    // return val; 
  ...
  static Node *copy_nodes_impl(Node *node) {
    ...
    //   copy_node->right = copy_node_impl(node->right);
    // }
  ...
  static void destroy_nodes_impl(Node *node) {
    ...
    delete node;
    //if (size_impl(node) == 1){ delete node; return; }
  ...
  static Node * insert_impl(Node *node, const T &item, Compare less) {
    ...
    return node;
    // insert_impl(node->left); 
    // if (less(*query, node->datum))
    // insert_impl(node->right);
  ...
  //helper func for check_sorting_invariant
  // static bool isBST_helper(Node *node, Compare less, int min, int max){
  //   if (node == nullptr){
  //     return true;
  //   }

  //   if (less(min,node->datum) || less(max, node->datum)){
  //     return false; 
  //   } 

  //   if (isBST_helper(node->left, less, min, node->datum - 1)){
  //     if (isBST_helper(node->right, less, node->datum + 1, max)){
  //       return true;
  //     }
  //   }

  //   return false; 
  //   // return isBSTstill(node->left, less, min, node->datum-1) &&
  //   //         isBSTstill(node->right, less, node->datum+1, max);
  // }
  ...
  static bool check_sorting_invariant_impl(const Node *node, Compare less) {
    ...
    // const int min = *min_element_impl(node);
    // const int max = *max_element_impl(node);
    // if (isBST_helper(node, min, max)){
    //   return true; 
    // }
    ...
    if (node->right != nullptr && less(max_element_impl(node->right)->datum, 
                                  node->datum)){ 
      return false; 
    }
  ...
  //helper func min_greater_than_impl
  // static std::vector<T> helper_inorder(const Node *node) {
  //   if (node == nullptr){ return inorder; }

  //   helper_inorder(node->left);
  //   std::vector<T> inorder = node->datum.push_back(); 
  ...
  static Node * min_greater_than_impl(Node *node, const T &val, Compare less) {
    ...
    if (node->left == nullptr && node->right == nullptr && less(node->datum, val)){
      return nullptr; 
    }
    ...
    if (less(val, node->datum) && !less(node->left->datum, val) 
        ...
        && node->left != nullptr && node->left->right != nullptr){
